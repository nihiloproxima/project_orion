...
const generateSpyReport = (ownerId: string): SpyMail => ({
  ...
  title: `Spy Report: Planet (${Math.floor(Math.random() * 100)}, ${Math.floor(
    Math.random() * 100
  )})`,
  ...
  data: {
    ...
    resources: {
      ...
      production: {
        ...
        deuterium: Math.floor(Math.random() * 500),
        microchips: Math.floor(Math.random() * 250),
        science: Math.floor(Math.random() * 100),
        energy_production: Math.floor(Math.random() * 2000),
        energy_consumption: Math.floor(Math.random() * 1500),
      },
      storage: {
        metal: 100000,
        deuterium: 50000,
...
const generateGameMessage = (ownerId: string): BaseMail => ({
  ...
  content:
    "Welcome, Commander! Your journey begins now. Check out the tutorial to get started.",
  read: Math.random() > 0.5,
  archived: false,
  data: {},
});
...
const generateMissionReport = (ownerId: string): BaseMail => ({
  ...
  content:
    "Your transport fleet has successfully delivered resources to the target planet.",
  read: Math.random() > 0.5,
  archived: false,
  data: {
...
export const generateTestMails = (
  ownerId: string,
  count: number = 20
): BaseMail[] => {
