...
export function ResearchCard({
	...
	const costMultiplier = Math.pow(1 + research.cost.percent_increase_per_level / 100, tech.level);

	const timeMultiplier = Math.pow(1 + research.time.percent_increase_per_level / 100, tech.level);
	...
	const prerequisitesMet = prerequisites.every((prereq) => tech.level >= prereq.required_level);
	...
	return (
		<div className="bg-gray-900/90 rounded-lg shadow-lg overflow-hidden border border-blue-500/30 hover:border-blue-500 transition-all">
			<div className="p-4">
				<div className="flex items-center mb-4">
					<div className="relative">
						<Image
							...
							className={`w-16 h-16 rounded mr-4 ${!prerequisitesMet ? 'opacity-50 grayscale' : ''}`}
						/>
						{!prerequisitesMet && (
					...
					<div>
						...
						<div className="flex items-center mt-1">
							<Progress value={(tech.level / research.max_level) * 100} className="w-32 h-2" />
				...
				{/* Research Status */}
				{tech.is_researching ? (
					<Timer startTime={tech.research_start_time!} finishTime={tech.research_finish_time!} />
				) : (
					<>
						<p className="text-sm text-gray-400 mb-4">{assetConfig?.description}</p>

						{/* Prerequisites Warning */}
						{!prerequisitesMet && (
							<div className="mb-4 p-2 bg-red-900/30 border border-red-500/30 rounded-md flex items-center gap-2">
								<AlertCircle className="w-4 h-4 text-red-500" />
								<span className="text-sm text-red-400">Prerequisites not met</span>
							</div>
						)}
						...
						<div className="grid grid-cols-2 gap-2 mb-4">{/* Resource cost items */}</div>

						<button
							onClick={() => onStartResearch(id)}
							disabled={!canAfford || !prerequisitesMet || tech.level >= research.max_level}
